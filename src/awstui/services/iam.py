...
class IAMPlugin(AWSServicePlugin):
        ...
        if category == "users":
            ...
            for page in paginator.paginate():
                for user in page.get("Users", []):
                    nodes.append(TreeNode(id=f"iam:user:{user['UserName']}", label=user["UserName"], node_type="user", service="iam", expandable=True, metadata={"user_name": user["UserName"]}))
            return nodes
        if category == "roles":
            ...
            for page in paginator.paginate():
                for role in page.get("Roles", []):
                    nodes.append(TreeNode(id=f"iam:role:{role['RoleName']}", label=role["RoleName"], node_type="role", service="iam", expandable=True, metadata={"role_name": role["RoleName"]}))
            return nodes
        if category == "policies":
            ...
            for page in paginator.paginate(Scope="Local"):
                for policy in page.get("Policies", []):
                    nodes.append(TreeNode(id=f"iam:policy:{policy['PolicyName']}", label=policy["PolicyName"], node_type="policy", service="iam", expandable=False, metadata={"policy_arn": policy["Arn"]}))
            return nodes
        if category == "groups":
            ...
            for page in paginator.paginate():
                for group in page.get("Groups", []):
                    nodes.append(TreeNode(id=f"iam:group:{group['GroupName']}", label=group["GroupName"], node_type="group", service="iam", expandable=False, metadata={"group_name": group["GroupName"]}))
            return nodes
        return []
    ...
    def _get_user_attached_policies(self, client, node: TreeNode) -> list[TreeNode]:
        ...
        return [TreeNode(id=f"iam:attached_policy:{p['PolicyArn']}", label=p["PolicyName"], node_type="attached_policy", service="iam", expandable=False, metadata={"policy_arn": p["PolicyArn"]}) for p in response.get("AttachedPolicies", [])]
    ...
    def _get_user_inline_policies(self, client, node: TreeNode) -> list[TreeNode]:
        ...
        return [TreeNode(id=f"iam:inline_policy:{node.metadata['user_name']}:{name}", label=name, node_type="user_inline_policy", service="iam", expandable=False, metadata={"user_name": node.metadata["user_name"], "policy_name": name}) for name in response.get("PolicyNames", [])]
    ...
    def _get_user_access_keys(self, client, node: TreeNode) -> list[TreeNode]:
        ...
        return [TreeNode(id=f"iam:access_key:{key['AccessKeyId']}", label=key["AccessKeyId"], node_type="access_key", service="iam", expandable=False, metadata={"user_name": node.metadata["user_name"], "access_key_id": key["AccessKeyId"]}) for key in response.get("AccessKeyMetadata", [])]
    ...
    def get_details(self, session: boto3.Session, node: TreeNode) -> ResourceDetails:
        ...
        if node.node_type == "user":
            ...
            user = response["User"]
            return ResourceDetails(title=f"IAM User: {user['UserName']}", subtitle=user.get("Arn", ""), summary={"User Name": user["UserName"], "User ID": user.get("UserId", ""), "ARN": user.get("Arn", ""), "Created": str(user.get("CreateDate", ""))}, raw=user)

        if node.node_type == "role":
            response = client.get_role(RoleName=node.metadata["role_name"])
            role = response["Role"]
            return ResourceDetails(title=f"IAM Role: {role['RoleName']}", subtitle=role.get("Arn", ""), summary={"Role Name": role["RoleName"], "Role ID": role.get("RoleId", ""), "ARN": role.get("Arn", ""), "Created": str(role.get("CreateDate", "")), "Description": role.get("Description", ""), "Max Session (s)": str(role.get("MaxSessionDuration", ""))}, raw=role)

        if node.node_type == "policy":
            response = client.get_policy(PolicyArn=node.metadata["policy_arn"])
            policy = response["Policy"]
            return ResourceDetails(title=f"IAM Policy: {policy['PolicyName']}", subtitle=policy.get("Arn", ""), summary={"Policy Name": policy["PolicyName"], "ARN": policy.get("Arn", ""), "Description": policy.get("Description", ""), "Attachment Count": str(policy.get("AttachmentCount", 0)), "Default Version": policy.get("DefaultVersionId", ""), "Created": str(policy.get("CreateDate", ""))}, raw=policy)

        if node.node_type == "group":
            response = client.get_group(GroupName=node.metadata["group_name"])
            group = response["Group"]
            users = response.get("Users", [])
            return ResourceDetails(title=f"IAM Group: {group['GroupName']}", subtitle=group.get("Arn", ""), summary={"Group Name": group["GroupName"], "Group ID": group.get("GroupId", ""), "ARN": group.get("Arn", ""), "Created": str(group.get("CreateDate", "")), "Members": ", ".join(u["UserName"] for u in users) or "None"}, raw=response)

        if node.node_type == "role_trust_policy":
            response = client.get_role(RoleName=node.metadata["role_name"])
            doc = response["Role"].get("AssumeRolePolicyDocument", {})
            return ResourceDetails(title=f"Trust Policy: {node.metadata['role_name']}", subtitle=f"IAM Role: {node.metadata['role_name']}", summary={"Policy Document": json.dumps(doc, indent=2)}, raw=doc)

        if node.node_type == "attached_policy":
            response = client.get_policy(PolicyArn=node.metadata["policy_arn"])
            policy = response["Policy"]
            return ResourceDetails(title=f"IAM Policy: {policy['PolicyName']}", subtitle=policy.get("Arn", ""), summary={"Policy Name": policy["PolicyName"], "ARN": policy.get("Arn", ""), "Description": policy.get("Description", "")}, raw=policy)

        if node.node_type == "user_inline_policy":
            response = client.get_user_policy(UserName=node.metadata["user_name"], PolicyName=node.metadata["policy_name"])
            doc = response.get("PolicyDocument", {})
            return ResourceDetails(title=f"Inline Policy: {node.metadata['policy_name']}", subtitle=f"User: {node.metadata['user_name']}", summary={"Policy Document": json.dumps(doc, indent=2)}, raw=response)

        if node.node_type == "role_inline_policy":
            response = client.get_role_policy(RoleName=node.metadata["role_name"], PolicyName=node.metadata["policy_name"])
            doc = response.get("PolicyDocument", {})
            return ResourceDetails(title=f"Inline Policy: {node.metadata['policy_name']}", subtitle=f"Role: {node.metadata['role_name']}", summary={"Policy Document": json.dumps(doc, indent=2)}, raw=response)

        if node.node_type == "access_key":
            return ResourceDetails(title=f"Access Key: {node.metadata['access_key_id']}", subtitle=f"User: {node.metadata['user_name']}", summary={"Access Key ID": node.metadata["access_key_id"], "User": node.metadata["user_name"]}, raw={"AccessKeyId": node.metadata["access_key_id"]})
